    ...
    try {
        ...
        if (link.active) { 
            ...
            try {
                ...
                if (!country) {
                    console.error('country info undefined');
...
router.post('/search', async (req, res) => {
    try {
        ...
        const links = await Link.find({ name: { $regex: searchQuery, $options: 'i' } });
...
router.post('/', requireLogin, isValidUrl, async (req, res) => {
    try {
        ...
        if (customKeyword) {
            ...
            if (existingLink) {
                return res.render('error', { error: 'Custom keyword is already taken, please enter something else.' });
            }
        } else {
